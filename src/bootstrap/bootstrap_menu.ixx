...
namespace bootstrap
{
    ...
    export bool InitializeMenu(const std::unique_ptr<C_ServiceContainer>& services)
    {
        const auto renderQueue = services->get<gui_renderer_queue_t>();
        const auto widgetRegedit = services->get<gui::C_WidgetRegedit>();
        ...
        services->add<render_hook_subscription_proxy_s>(
            C_ServiceLocator::Get<hook::C_HookDispatcher>()->subscribe<ImDrawList*>(static_cast<hook::hook_id_t>(hook::hook_type_e::ON_IMGUI_RENDER),
