...
namespace bootstrap
{
    ...
    bool SetupWorldStateHook()
    {
        const auto onLevelInit = memory::OffsetManager::GetOnLevelInit();
        if (!onLevelInit) {
        ...
        const auto onLevelShutdown = memory::OffsetManager::GetOnLevelShutdown();
        if (!onLevelShutdown) {
