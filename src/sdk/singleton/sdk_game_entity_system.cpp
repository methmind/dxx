...
namespace sdk::singleton
{
    bool C_GameEntitySystem::findInstance()
    {
        ...
        this->identitiesChunks_ = reinterpret_cast<iface::entity_identities_chunk_s**>(static_cast<uint8_t*>(this->instance_) + 0x10);
    ...
    {
        if (index < 0 || index >= iface::MAX_TOTAL_ENTITIES) {
            return nullptr;
        }

        const auto chunkIndex = index / iface::MAX_ENTITIES_IN_CHUNK;
        const auto chunk = this->identitiesChunks_[chunkIndex];
        if (!chunk) {
            return nullptr;
        }

        const auto indexInChunk = index % iface::MAX_ENTITIES_IN_CHUNK;
