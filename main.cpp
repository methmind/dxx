...
int main(int, char**)
{
    auto container = bootstrap::C_ApplicationBuilder::Build();
    if (!container) {
    ...
    auto settingsForm = std::dynamic_pointer_cast<menu::C_MenuSettingsForm>(
        guiRegedit->createWidget<menu::C_MenuSettingsForm>(guiRegedit, container->get<lua::C_LuaScriptManager>())
    );

    auto mainForm = std::dynamic_pointer_cast<menu::C_MenuMainForm>(
    ...
    io.Fonts->AddFontFromFileTTF("C:/Windows/Fonts/Verdana.ttf", 15.0f); // Load font at 18 pixels
